...
MAJH OS is the platform. MAJH Events is the primary tenant. Four departments under MAJH Events (Esports, CarBadMV, Tradewinds RB, T.R.S.). External tenants (LGS, hotels) will be separate top-level tenants.
Schema supports four-level model (Platform → Tenant → Department → Location). Application authorization currently checks tenant-scoped roles only; department and location scoping is schema-ready but not wired.
Next migration phase: financial table scoping (department_id and location_id on ledger_transactions). Then CarBadMV operational features. Then Wizard UI. Then full authorization migration.
