...
## "Four-Level Architecture (May 18, 2026)." 

Four-Level Architecture (added May 18, 2026)
The platform supports a four-level organizational model: Platform → Tenant → Department → Location. Tables added or modified in this migration establish the Department and Location levels. The application code does not yet consume these new columns for authorization — that wiring is tracked as T-204 in BACKLOG.md.
departments table
Operating lines of business within a tenant. A tenant may have multiple departments; each department belongs to exactly one tenant.
ColumnTypeNotesiduuidPrimary key, default gen_random_uuid()tenant_iduuidFK → tenants(id), ON DELETE CASCADE, NOT NULLslugtextURL-safe identifier, unique within tenantnametextDisplay namedescriptiontextOptionalis_activebooleanDefault truesort_orderintegerDefault 0, for UI orderingcreated_attimestamptzDefault now()updated_attimestamptzDefault now()
Constraints: UNIQUE (tenant_id, slug).
Indexes: idx_departments_tenant on tenant_id.
RLS: Members of a tenant can SELECT their tenant's departments. Tenant owners (role_key = 'owner') can ALL.
Current seed data: 4 rows under MAJH Events tenant — esports, carbadmv, tradewinds-rb, trs.
locations table
Geographic or physical instances of a department. A department may have multiple locations (e.g., CarBadMV-DC, future CarBadMV-NYC). Each location belongs to exactly one department.
ColumnTypeNotesiduuidPrimary key, default gen_random_uuid()tenant_iduuidFK → tenants(id), ON DELETE CASCADE, NOT NULLdepartment_iduuidFK → departments(id), ON DELETE CASCADE, NOT NULLslugtextURL-safe identifier, unique within departmentnametextDisplay namecurrencyvarchar(3)ISO 4217 code, default 'USD'timezonetextIANA timezone, default 'UTC'tax_ratenumeric(5,4)Default 0.0000is_activebooleanDefault truecreated_attimestamptzDefault now()updated_attimestamptzDefault now()
Constraints: UNIQUE (department_id, slug).
Indexes: idx_locations_tenant on tenant_id, idx_locations_department on department_id.
RLS: Members of a tenant can SELECT their tenant's locations. Tenant owners can ALL.
Currency, timezone, and tax_rate are location-scoped because: regional accounting accuracy depends on these values matching the physical location. A Tradewinds-RB Barbados transaction calculates VAT at 17.5% in BBD; a Tradewinds-RB St. Lucia transaction calculates VAT at 12.5% in XCD. Financial reporting requires location-level isolation.
Current seed data: 5 rows — carbadmv/dc-metro (USD), tradewinds-rb/barbados (BBD), tradewinds-rb/st-lucia (XCD), trs/bgi-airport (BBD), esports/digital (USD).
Authorization scoping columns
Three existing authorization tables received scoping columns to support tenant-defined and department-defined role templates. All new columns are nullable to preserve backward compatibility with existing rows.
organization_role_templates — added columns
ColumnTypeNotestenant_iduuid (nullable)FK → tenants(id), ON DELETE CASCADEdepartment_iduuid (nullable)FK → departments(id), ON DELETE CASCADE
Semantics:

Both NULL → platform-wide default role template (current 9 system roles).
tenant_id set, department_id NULL → role defined by a specific tenant for use across that tenant.
Both set → role defined by a specific department, scoped to that department only.

The 9 existing system role templates (owner, admin, manager, staff, member, sponsor, venue, vendor, observer) remain with both columns NULL.
Indexes: idx_org_role_templates_tenant, idx_org_role_templates_dept.
role_template_permissions — added columns
ColumnTypeNotestenant_iduuid (nullable)FK → tenants(id), ON DELETE CASCADEdepartment_iduuid (nullable)FK → departments(id), ON DELETE CASCADE
Semantics: Same as organization_role_templates above. A permission mapping inherits its scope from the role template it belongs to. Existing 103 rows have both columns NULL (mapping platform-default roles to platform-default permissions).
Indexes: idx_role_template_perms_tenant, idx_role_template_perms_dept.
organization_members — added columns
ColumnTypeNotesdepartment_iduuid (nullable)FK → departments(id), ON DELETE SET NULLlocation_iduuid (nullable)FK → locations(id), ON DELETE SET NULL
Semantics: A member's role scope is defined by which columns are populated.

department_id NULL, location_id NULL → tenant-wide role (e.g., MAJH OS tenant owner).
department_id set, location_id NULL → department-wide role (e.g., CarBadMV manager).
Both set → location-scoped role (e.g., CarBadMV DC-Metro bartender).

Tenant-level roles inherit permissions down to all departments and locations within that tenant. Department-level roles do NOT inherit up to tenant scope.
Existing 2 rows (both MAJH Events owners) have both new columns NULL, retaining tenant-wide scope.
Indexes: idx_org_members_dept, idx_org_members_location.
Important: code/schema gap
As of May 18, 2026, the application code does not yet read these new scoping columns for authorization decisions. The existing single-tenant authorization paths (requireRole() querying staff_roles, inline staff_roles queries in 107+ places, profiles.role checks in 12+ places — 144 total check points) remain operative. The schema is ready for the migration; the migration itself is tracked as T-204.
Until T-204 ships, the new columns are inert metadata. Setting department_id on an organization_members row does NOT yet restrict that member's access to a department. The schema supports the intent; the code does not yet enforce it.
